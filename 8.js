console.log("Simple numbers in test data: ", findUniques(parseData(testSegments())));
console.log("Simple numbers in data: ", findUniques(parseData(segments())));

console.log("Final output from test data: ", getDataOutput(parseData(testSegments())));
console.log("Final output from data: ", getDataOutput(parseData(segments())));

function findUniques(s) {
  return s.reduce((c, e) => c + e[1].filter(f => [2, 3, 4, 7].includes(f.length)).length, 0);
}

function getDataOutput(data) {

  return data.reduce((c, s) => c + decodeOutput(s), 0);
...
function decodeOutput(s) {
  ...
  //assign uniques
  uniques.forEach(u => {
    symbolDefs[u[0]] = s[0].find(y => y.length === u[1]);
  })

  //Get 5 segments
  const fiveSeg = s[0].filter(x => x.length === 5);

  //Get 3
  fiveSeg.forEach(x => {
    if (diffStrings([x, symbolDefs[1]]).length === 3) {
      symbolDefs[3] = x;
    }
  });
  ...
  //Get 2
  fiveSeg.forEach(x => {
    if (diffStrings([x, symbolDefs[4], symbolDefs[3], ]).length === 1) {
      symbolDefs[2] = x;
    }
  });
  ...
  //Get 5
  fiveSeg.forEach(x => {
    if (![symbolDefs[2], symbolDefs[3]].includes(x)) {
      symbolDefs[5] = x;
    }
  });

  //Get 6 segments
  const sixSeg = s[0].filter(x => x.length === 6);

  //Get 0
  sixSeg.forEach(x => {
    if (diffStrings([x, symbolDefs[5]]).length === 2) {
      symbolDefs[0] = x;
    }
  });
  ...
  //Get 9
  sixSeg.forEach(x => {
    if (diffStrings([x, symbolDefs[4]]).length === 2) {
      symbolDefs[9] = x;
    }
  });
  ...
  //Get 6
  sixSeg.forEach(x => {
    if (![symbolDefs[0], symbolDefs[9]].includes(x)) {
      symbolDefs[6] = x;
    }
  });
  ...
  for (let i = 0; i <= 9; i++) {
    lookup[symbolDefs[i]] = `${i}`;
  }

  const output = parseInt(
    s[1].reduce((c, e) => c + lookup[e], "")
  )

  return output;
}

function diffStrings(arr) {
  return arr.slice(1).reduce((c, s) => c.split("").filter(e => !s.split("").includes(e)).join(""), arr[0]);
}
...
function parseData(input) {
  return input.replaceAll("|\n", "| ")
    ...
    .map(e => {
      return e.split("|")
        .map(f => f.trim().split(" ").map(g => g.split("").sort().join("")))
    });
}

function single() {
  return `acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf`.split("|")
    .map(f => f.trim().split(" ").map(g => g.split("").sort().join("")))
}

function testSegments() {
  return `be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb |
...
function segments() {
  return `dac abcf ac fdbcga dgcbae gcbfde fgcbd agfed adcgf cdbgfea | cbfa bcafdg cbfa bafcgd
